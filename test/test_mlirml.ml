...
let%test "matmul" =
  with_context (Context.get None true) (fun ctx ->
    ...
    let iterator_types = [ "parallel"; "reduction" ] in
    let dcsr = SparseTensorEncodingAttr.get
      ctx
      [ 
      ...
      in
    let index_type = Type.index ctx in
    let tensor_type = RankedTensorType.get [ ShapedType.Dynamic; ShapedType.Dynamic ] float64 (Some dcsr) location in
    let m = Module.empty location in
    m#body#append_operation @@ Func.func
      ctx
      (StringAttr.get ctx "matmul")
      (FunctionType.get ctx [ tensor_type; tensor_type ] [ tensor_type ] |> TypedAttr.get)
      []
      location
      ~init:(fun body ->
  ...
  = "#map = affine_map<(d0, d1) -> (d0, d1)>\n\
    ...
    module {\n\
    \  func.func @matmul(%arg0: tensor<?x?xf64, #sparse>, %arg1: tensor<?x?xf64, #sparse>) -> tensor<?x?xf64, #sparse> {\n\
    \    %c0 = arith.constant 0 : index\n\
    \    %c1 = arith.constant 1 : index\n\
    \    %0 = tensor.empty(%c0, %c1) : tensor<?x?xf64, #sparse>\n\
    \    %1 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = [\"parallel\", \"reduction\"]} ins(%arg0, %arg1 : tensor<?x?xf64, #sparse>, tensor<?x?xf64, #sparse>) outs(%0 : tensor<?x?xf64, #sparse>) {\n\
