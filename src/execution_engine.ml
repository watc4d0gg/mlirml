...
module ExecutionEngine = struct
  ...
  class t raw =
    object (self)
      ...
      method dump_to_object_file name = mlir_execution_engine_dump_to_object_file self#raw (mlir_string_ref_create_from_cstring name)
      method raw = raw
    end
  ...
  let get m opt_level shared_lib_paths dump_object_file =
    let lib_array = CArray.of_list MlirStringRef.t (List.map mlir_string_ref_create_from_cstring shared_lib_paths) in
    mlir_execution_engine_create m#raw opt_level (CArray.length lib_array) (CArray.start lib_array) dump_object_file |> from_raw
end
