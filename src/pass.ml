...
module ExternalPass = struct
  ...
  let create id name arg description op dialects ~on_construct ~on_destruct ~on_clone ~init ~run =
    ...
    let callbacks = Ctypes.make MlirExternalPassCallbacks.t in
    ...
    in
    setf callbacks MlirExternalPassCallbacks.construct construct_funptr;
    setf callbacks MlirExternalPassCallbacks.destruct destruct_funptr;
    setf callbacks MlirExternalPassCallbacks.clone clone_funptr;
    setf callbacks MlirExternalPassCallbacks.initialize init_funptr;
    setf callbacks MlirExternalPassCallbacks.run run_funptr;
...
module OpPassManager = struct
  class t raw = object (self)
    method nested_under op = mlir_op_pass_manager_get_nested_under self#raw (mlir_string_ref_create_from_cstring op) |> new t
...
module PassManager = struct
  ...
  class t raw = object (self)
    ...
    method nested_under op = mlir_pass_manager_get_nested_under self#raw (mlir_string_ref_create_from_cstring op) |> OpPassManager.from_raw
