...
module Rewriter = struct
  class t raw = object (self)
    ...
    method insert (op : Operation.t) = mlir_rewriter_base_insert self#raw op#raw
    method clone (op : Operation.t) = mlir_rewriter_base_clone self#raw op#raw
    method clone_without_regions (op : Operation.t) = mlir_rewriter_base_clone_without_regions self#raw op#raw
