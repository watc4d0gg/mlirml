...
module TranslationUnit = struct
  let parse index source_filename command_line_args unsaved_files ~flags =
    ...
    let result =
      clang_parse_translation_unit
        index
        source_filename
        (CArray.start arg_array)
        (CArray.length arg_array)
        (CArray.start files_array)
        (Unsigned.UInt.of_int (CArray.length files_array))
        (Unsigned.UInt.of_int64 mapping)
    in
    if Ctypes.is_null result then None else Some result


  let cursor translation_unit =
    let result = clang_get_translation_unit_cursor translation_unit in
...
module Cursor = struct
  ...
  let visit_children cursor visitor =
    let funptr =
      coerce
        ...
        CXCursorVisitor.t
        (fun child _ _ -> visitor child cursor)
    in
  ...
  let underlying_type cursor =
    if kind cursor = CXCursor_TypedefDecl
    then Some (clang_get_typedef_decl_underlying_type cursor)
    else None


  let is_bit_field cursor =
    if clang_cursor_is_bit_field cursor = Unsigned.UInt.zero then false else true
end

module SourceLocation = struct
  let is_in_system_header source_location =
    if clang_location_is_in_system_header source_location == 0 then false else true


  let is_from_main_file source_location =
    if clang_location_is_from_main_file source_location == 0 then false else true
end
...
module CType = struct
  ...
  let get_constant_array_data ctype =
    ...
    if kind element_type = CXType_Invalid
    then None
    else (
      let size = clang_get_array_size ctype |> Signed.LLong.to_int64 in
      if size = Int64.of_int (-1) then None else Some (element_type, size))
end
...
let with_index excludeDeclarationsFromPCH displayDiagnostics f =
  ...
  | Some index -> Fun.protect ~finally:(fun _ -> Index.dispose index) (fun _ -> f index)
...
let with_translation_unit filename command_line_args f index =
  ...
  | None ->
    failwith
      (Printf.sprintf "Unable to initialize a translation unit for the file %s!" filename)
...
let with_cursor f translation_unit =
  ...
  | None ->
    failwith (Printf.sprintf "Unable to retrieve the cursor from the translation unit!")
